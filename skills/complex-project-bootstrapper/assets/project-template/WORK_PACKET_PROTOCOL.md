...
A profile may register domain rules over its `domain` block in `DOMAIN_MODULES` in `scripts/work_packet.py`; `validate_contract` applies them after the common checks wherever a contract is authored — `create` and `revise`, feedback contract repair and a new acceptance initialization — so a profile rule cannot be bypassed by choosing the path. A revision stored before the rules existed replays (ADR-037): `validate`, `graph` and `render` report it as valid history qualified by a `DOMAIN SHORTFALL` line naming the failing revisions, never as clean, and its next revision must comply. The profile is a required argument of `validate_contract` and must be one of the schema's registered profiles: there is no default, so a caller cannot skip a profile's rules by omitting it. A registered module may close its `domain` block; the `software-hardware` block admits only its declared keys and refuses the literal `VERIFIED_ON_HARDWARE` in any string it carries. The `software-hardware` profile registers `scripts/software_hardware.py`: one named component per packet, hardware assumptions citing contract sources, every validation mapped to one rung of the verification ladder, a declared `command` required at machine rungs and refused at hardware rungs, and `hardware_status` limited to `UNVERIFIED_ON_HARDWARE` or `NOT_HARDWARE_FACING` because `VERIFIED_ON_HARDWARE` is earned in the acceptance ledger, never authored. `examples/software-hardware/` is the worked decomposition.
