...
All connector mutations require current ACTIVE approval and the bound configuration digest. The initial approval anchor remains fixed in the registry. An architecture change stops this registry's writes until an explicitly reviewed continuity/migration procedure preserves its history; silently creating a second registry is forbidden. Read-only audit/recovery works without activation, and only the identity-bearing configuration — schema version, repository, state branch, accepted branch and master issue — is frozen into the registry, so rotating publishers or disabling writes leaves an existing registry readable. Each completed receipt records its own author, so a rotation applies to new publications without invalidating receipts an earlier publisher already wrote. Recovery validates a fetched payload against the operator's configuration rather than the copy embedded in it, and refusal follows the decoded payload size rather than the size GitHub reports. Recording local feedback after revocation still works under the feedback protocol; keep those pending records for authorized later publication.
...
The illustration is not a valid executable packet. Use `work_packet.py` to create/validate the complete packet. Populate `feedback` with the original parsed event files in sequence; never paste a shortened rendering in place of events. Once feedback exists, use its derived packet until it reaches `REVIEW_PENDING`, or `ACCEPTED` once the acceptance controller has recorded its decision through `sync-feedback` (ADR-025). Only validated reviewer/integrator event extensions may follow that exact packet history; pending dispatches and human holds cannot be overridden by another valid-looking packet. Existing task/issue and implementation branch bindings cannot be replaced. A PR binding is replaced only through supersession: append the outgoing number to `superseded_prs` in the same write that changes or clears `pr`, one per change, never on an accepted row. Every superseded number must be a live PR that is closed and was not merged, so a merged PR cannot be hidden by superseding it, and no other task may adopt a superseded number. A task's first registration must carry an empty `superseded_prs`: supersession history is only ever created by performing a replacement, never asserted at intake. Branches must use the supported ASCII subset of valid Git ref names, including valid path components. No configured or registered branch may be a path-component prefix of another, because Git cannot hold `refs/heads/a` and `refs/heads/a/b` at the same time. Full packet and feedback history prefixes must remain intact. Every registry read and write validates all registered packets as one complete acyclic dependency graph. The registry holds exactly one domain profile, and registration, publication and closure require it to match the approved project's profile. Register prerequisites first; READY and later dependent packets require VERIFIED prerequisites. A prerequisite cannot regress while its dependents remain in an advanced state. Accepted rows are immutable; follow-up work gets a new task.
...
A publication may only project the row being committed: the newest entry for a task must equal that task's current projection, and any entry appended in a write must be the projection of the row that write commits. A summary that matches no committed row is refused on every read and write, so a fabricated acceptance cannot be posted as a canonical record. Each changed task creates a deterministic publication intent. Publication moves through three committed phases, each by file-SHA comparison: `claimed` takes the exclusive claim, `posting` records that a POST is about to be attempted, and `done` records the receipt and its author. The comment is posted only from `posting`, so a `claimed` entry is durable proof that no POST was attempted and any publisher may resume it; an interrupted claim therefore never strands a task. Its marker, author, exact body and pinned state-commit reference are verified across all comment pages. A second publisher cannot claim the same operation, and no entry may move backwards.

```text
python scripts/github_ledger.py --config config/github-ledger.json --root . reconcile
```

If GitHub accepted the comment but its response was lost, reconciliation finds it and records the receipt without posting again. If a claimed operation has no visible comment, it stays uncertain and **is not automatically retried**. Wait for visibility and reconcile. A `posting` entry with no visible comment stays uncertain indefinitely and is never resumed or retried: full pagination proves only present absence, and a comment that GitHub accepted may become visible later, so retrying could publish a second copy. Resolving it needs GitHub evidence and an operator decision, and the adapter offers no command that retires such a record. `reconcile` requires the same approval, bound configuration and allowed publisher identity as publication, because it commits registry writes too. Conflicting, edited, deleted, duplicated or untrusted receipts fail audit. The adapter never deletes/edits comments or silently releases claims. API failures, truncated pagination and size limits fail closed. These guarantees cover supported clients, not a repository administrator rewriting history or hostile credentials.
