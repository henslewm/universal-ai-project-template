...
4. Declare each harness's own `context_overhead_tokens` — what it consumes before the brief is read — and `limits.chars_per_token`, the conservative divisor used to estimate the brief. Where a resource is served in a smaller window than the router configuration declares, state that window in the binding's `served_context_window`; a binding may never claim more than the resource declares. Measure the overhead against the installed version. On Cline CLI 3.86.2 the system prompt and tool definitions were observed to consume roughly 6805 tokens before any brief, which is most of an 8192-token window on its own.
5. Dispatch preparation requires `enabled: true`. Everything else — approval, the bound configuration digest, the attempt budget — is enforced by the controllers this harness defers to.
...
Preparation also checks capacity, which no controller above it can. The router compares the architect's declared token estimate against the resource's declared context window; it cannot see what the harness adds afterwards — the rendered brief, the worker rules and the harness's own system prompt — nor that a locally hosted model is served in the window it happened to be loaded with. So `dispatch` sums the declared harness overhead, the measured brief, the rules and the reserved output tokens, and refuses when that exceeds the served window, reporting the arithmetic as `context_estimate` on a prepared dispatch. The estimate is a character-count approximation and deliberately conservative; it is not a tokenizer, and an understated `context_overhead_tokens` or an overstated `served_context_window` will still let an impossible dispatch through. Its purpose is to turn a failure the worker meets on its first tool call into a refusal with stated numbers.
